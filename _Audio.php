...
class Audio {
    ...
    public static function radio() {
        ...
        $radio = [
            ...
            'content' => $content,
            'src' => 'http://oneway.out.airtime.pro:8000/oneway_a',
    ...
    public static function listAudioCate($id,$from,$limit = null, $sort=null) {
        ...
        if ($sort == 'view') {
			...
			$arg['orderby'] = 'meta_value_num';
            $arg['meta_key'] = 'oneway_like';
            $arg['order'] = 'DESC';
			
		} elseif ($sort == 'share') {
			$arg['orderby'] = 'meta_value_num';
            $arg['meta_key'] = 'oneway_share';
            $arg['order'] = 'DESC';
...
//				}
    ...
    public static function listComment($id) {
        ...
        // Count comment(s) in post
        $count = get_comments(['post_id' => $id,'count'=> true, 'status' => 'approve',]);
        
        if (intval($count) == 0 || $count == null) { return []; }

        //Set defauld $limit is 100
        $limit = 100;
        ...
        if(count($pre) > 0) {
            foreach($pre as $p) {
                ...
                $p['avatar'] = 'http://www.gravatar.com/avatar/'.md5( strtolower( trim( $p['email'] ) ) ).'?d=retro';
                $output[] = $p;
            }
